...
`V18__booking_per_employee.sql`:

```sql
-- 1. Backfill: assign legacy NULL employee_id on active bookings
--    Strategy: pick the first active employee of the tenant (single-employee tenants
--    have exactly one candidate; this is also the case once "default self-employee"
--    auto-creation is in place — see 2026-05-11-default-self-employee-design.md).
--    If no active employee exists, mark as CANCELLED with a dedicated reason.
UPDATE CARE_BOOKINGS b
   SET EMPLOYEE_ID = (
     SELECT MIN(e.ID) FROM EMPLOYEE e WHERE e.ACTIVE = 1
   )
...
-- 2. Drop the old global slot uniqueness
ALTER TABLE CARE_BOOKINGS DROP CONSTRAINT UK_BOOKING_SLOT;

-- 3. Per-employee uniqueness on active statuses (Oracle: function-based unique index)
CREATE UNIQUE INDEX UK_BOOKING_SLOT_EMPLOYEE ON CARE_BOOKINGS (
  CASE WHEN STATUS IN ('PENDING','CONFIRMED')
       THEN APPOINTMENT_DATE END,
  CASE WHEN STATUS IN ('PENDING','CONFIRMED')
       THEN APPOINTMENT_TIME END,
  CASE WHEN STATUS IN ('PENDING','CONFIRMED')
...
CREATE TABLE EMPLOYEE_ABSENCE (
  ...
  CONSTRAINT FK_EMPLOYEE_ABSENCE_EMPLOYEE FOREIGN KEY (EMPLOYEE_ID) REFERENCES EMPLOYEE(ID),
  CONSTRAINT CK_EMPLOYEE_ABSENCE_RANGE CHECK (END_AT > START_AT)
);
CREATE INDEX IDX_EMPLOYEE_ABSENCE_LOOKUP ON EMPLOYEE_ABSENCE (EMPLOYEE_ID, START_AT, END_AT);
```

Mirror in `migrateOracleSchema()` Java path for legacy tenants (idempotent `CREATE TABLE IF NOT EXISTS` style — see `feedback_legacy_tenant_create_table.md`).

The column `CANCELLATION_REASON VARCHAR2(64)` does not exist on `CARE_BOOKINGS` today; V18 adds it before running the backfill:
```sql
ALTER TABLE CARE_BOOKINGS ADD CANCELLATION_REASON VARCHAR2(64);
```

### 4.2 Domain — `Absence` entity
```java
@Entity @Table(name="EMPLOYEE_ABSENCE")
public class Absence {
  @Id @GeneratedValue Long id;
  @Column(nullable=false) Long employeeId;
  @Column(nullable=false) Instant startAt;
  @Column(nullable=false) Instant endAt;     // Instant + Oracle TIMESTAMP WITH TIME ZONE (per feedback_instant_oracle_timestamp_tz)
  String reason;
  @Column(nullable=false) Instant createdAt;
}
```

Repository: `AbsenceRepository extends JpaRepository<Absence,Long>` with `findOverlapping(employeeId, from, to)`.
...
**`POST /api/pro/employees/{id}/absences`** + `GET` + `DELETE /api/pro/employees/{id}/absences/{absenceId}` — new CRUD, gated by `@RequiresFeature(ABSENCE_MGMT)`.
...
- New `EmployeeAbsenceManagerComponent` under `features/employees/` — list / add / delete. Gated by `*lpFeatureEnabled="'ABSENCE_MGMT'"`.
